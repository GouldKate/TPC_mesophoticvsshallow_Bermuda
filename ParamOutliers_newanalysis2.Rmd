...
raw.data<-read.csv("C:/Github/TPC_BDA_github/R_output/Final/final_NP_R_GP_edited.csv")
View(raw.data)
```
...
mydataP$umol.cm2.hr<-abs(mydataP$umol.cm2.hr)
...
#save
#write.csv(mydata,"C:/Github/TPC_BDA_github/R_output/Final/October_RmarkdownFinal/mydata.csv")
```
...
```{r, include=FALSE}
#mydata<-read.csv("C:/Github/TPC_BDA_github/R_output/Final/October_RmarkdownFinal/mydata.csv")
...
# Filter to identify outliers using CooksSD at each temperature within each rate type (GP, R)
...
mPGPsp<-aov(Pmax~species, testRP)
summary(mPGPsp)

ggline(testGPP, "species", "Pmax",color= "species", add=c("mean_se", "dotplot"))
ggline(testRP, "species", "Pmax",color= "species", add=c("mean_se", "dotplot"))
...
#remove 13 SA1,15 SA3,19 SF1
testGPE<-testGP[-c(13,15,19),]
...
#remove 4 MC1
testRE<-testR[-(4),]
mPGPsp<-aov(E~species*treatment, testRE)
summary(mPGPsp)
...
res.aov3 <- aov(E ~ species * treatment, data = testGPEs)
res.aov3r <- aov(E~ species * treatment, data = testREs)
...
#remove 5MC2 6 MC3, 8 MF2
testREh<-testR[-c(5,6,8),]
...
res.aov3 <- aov(Eh ~ species * treatment, data = testGPEhs)
res.aov3r <- aov(Eh~ species * treatment, data = testREht)
...
pairwise.t.test(testGPEhs$Eh, testGPEhs$species,
                p.adjust.method = "BH")
pairwise.t.test(testREht$Eh, testREht$species,
                p.adjust.method = "BH")
##ANOVA assumes that the data are normally distributed and the variance across groups are homogeneous. We can check that with some diagnostic plots.
...
plot(res.aov3, 1) 
plot(res.aov3r, 1)
leveneTest(Eh ~ species* treatment, data = testGPEhs)
leveneTest(Eh ~ species* treatment, data = testREht)
#no evidence to suggest that the variance across groups is statistically significantly different. Therefore, we can assume the homogeneity of variances in the different treatment groups.
...
#remove 13 SA1 15 SA3
testGPTh_Ct<-testGP[-c(13,15),]
...
res.aov3 <- aov(Th_C ~ species * treatment, data = testGPTh_Ct)
res.aov3r <- aov(Th_C~ species * treatment, data = testRTh_Cs)
...
plot(res.aov3, 1) 
plot(res.aov3r, 1)
leveneTest(Th_C ~ species* treatment, data = testGPTh_Ct)
leveneTest(Th_C ~ species* treatment, data = testRTh_Cs)
#no evidence to suggest that the variance across groups is statistically significantly different. Therefore, we can assume the homogeneity of variances in the different treatment groups.
...
mPGPsp<-aov(log.Th_C~treatment,testGPTh_Ct)
summary(mPGPsp)
## check for outliers
cooksd <- cooks.distance(mPGPsp)
plot(cooksd, pch=".", cex=2, main="Influential Obs by Cooks distance")  # plot cook's distance
...
#remove 18 SF1 21 SL1 
testGPTh_Ct2<-testGPTh_Ct[-c(18,21),]
mPGPsp<-aov(log.Th_C~treatment,testGPTh_Ct2)
summary(mPGPsp)
# Extract the residuals
aov_residuals <- residuals(object = mPGPsp )
# Run Shapiro-Wilk test
shapiro.test(x = aov_residuals )
leveneTest(log.Th_C~treatment, testGPTh_Ct2) 


### log parameter E R- failed shapiro
testRTh_Cs$log.Th_C <- log(testRTh_Cs$Th_C+ 1) 

mPGPsp<-aov(log.Th_C~species*treatment,testRTh_Cs)
summary(mPGPsp)
# not significant 
# Extract the residuals
aov_residuals <- residuals(object = mPGPsp )
# Run Shapiro-Wilk test
shapiro.test(x = aov_residuals )
leveneTest(log.Th_C~treatment, testRTh_Cs) 


### make tables of pvalues for interaction GRoss photo

res.aov3<-aov(Th_C~species*treatment,testGPTh_Ct)
sstable<-Anova(res.aov3, type=2)
sstable
...
mPGPsp<-aov(log.Th_C~species,testRTh_Cs)
summary(mPGPsp)
# not significant 
# Extract the residuals
aov_residuals <- residuals(object = mPGPsp )
...
# Run Shapiro-Wilk test
shapiro.test(x = aov_residuals )
leveneTest(log.Th_C~treatment, testGPTh_Ct) 


### log parameter E R- failed shapiro
testRTh_Cs$log.Th_C <- log(testRTh_Cs$Th_C+ 1) 

mPGPsp<-aov(log.Th_C~treatment,testRTh_Cs)
summary(mPGPsp)
# not significant 
# Extract the residuals
aov_residuals <- residuals(object = mPGPsp )
...
res.aov3 <- aov(lnc ~ species * treatment, data = testGPlnct)
res.aov3r <- aov(lnc~ species * treatment, data = testRlncs)
...
plot(res.aov3, 1) 
plot(res.aov3r, 1)
leveneTest(lnc ~ species* treatment, data = testGPlnct)
leveneTest(lnc ~ species* treatment, data = testRlncs)
#no evidence to suggest that the variance across groups is statistically significantly different. Therefore, we can assume the homogeneity of variances in the different treatment groups.
